...
export class ConceptoFacturaService {
  ...
  async create(createConceptoFacturaInput: CreateConceptoFacturaInput) {
    ...
    const createInput = await this.createNewForFactura(
      createConceptoFacturaInput,
    );

    try {
      const createConceptoFacturaPayload = await prisma.conceptoFactura.create({
        data: {
          factura: {
            connect: {
              id: facturaId,
  ...
  async createNewForFactura(
    ...
    const { cantidad, productoId, servicioId, servicio } = conceptoFactura;
    let precio;
