...
import { CreateUsuarioInput, UpdateUsuarioInput } from 'src/types/graphql';
...
export class UsuarioService {
  async create(createUsuarioInput: CreateUsuarioInput) {
    const usuarioInput = new CreateUsuarioInput();
    const usuarioKeys = Object.keys(usuarioInput);
    for (const property of usuarioKeys) {
  ...
  async remove(usuarioDeleteArgs: Prisma.UsuarioDeleteArgs) {
    try {
      const usuarioDeleted = await prisma.usuario.delete(usuarioDeleteArgs);
