...
export class FacturaService {
  ...
  async create(createFacturaInput: CreateFacturaInput): Promise<Factura> {
    ...
    try {
      const createFacturaPayload = await prisma.$transaction(async (tx) => {
  ...
  async cancel(id: string) {
    ...
    // Estacion disponible
    try {
      const cancelFacturaPayload = await prisma.$transaction(async (tx) => {
        ...
        const pagoConnected =
          factura.pago !== null && factura.pago !== undefined;
        const eventoConnected =
          factura.evento !== null && factura.evento !== undefined;
  ...
  async save(facturaId: string) {
    try {
      const conceptosFactura =
        await this.conceptoFacturaService.findAllFromFactura(facturaId);
  ...
  getTotal(conceptosFactura: any[]) {
    ...
    if (productos.length > 0) {
      const availableProductos = productos.filter(
        async (producto) =>
          await this.productoService.isAvailable(
            producto.id,
            producto.cantidad,
          ),
      );
  ...
  calculateTotal(conceptosFactura: ConceptoFactura[]): number {
    return conceptosFactura.reduce((acc, current) => {
  ...
  async terminate(id: string, includeObject: any) {
    try {
      const conceptosFactura =
        await this.conceptoFacturaService.findAllFromFactura(id);
      const productosConceptos = conceptosFactura.filter(
        (cp) => !(cp.producto === null),
      );
      if (productosConceptos.length > 0) {
        productosConceptos.map((cf) =>
          this.productoService.substractFromInventory(
            cf.producto?.id ?? '',
            cf.cantidad,
          ),
        );
